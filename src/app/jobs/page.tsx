...
export const metadata: Metadata = {
  title: "فرصت‌های شغلی",
  description: "جست‌وجوی عمومی فرصت‌های شغلی کارجو از ارائه‌دهنده‌های مختلف.",
...
export default async function PublicJobsPage({
  ...
  return (
    <main className="min-h-dvh bg-surface">
      <header className="sticky top-0 z-30 border-b border-border/70 bg-background/85 backdrop-blur-md">
        <div className="dash-container flex h-16 items-center justify-between gap-4">
          ...
          <nav className="hidden items-center gap-5 text-sm font-semibold text-muted md:flex">
            <Link href="/jobs" className="text-foreground">فرصت‌های شغلی</Link>
            <Link href="/#boards" className="hover:text-foreground">ارائه‌دهنده‌ها</Link>
      ...
      <div className="dash-container py-8">
        <div className="space-y-6">
          <PageHeader
            title="فرصت‌های شغلی"
            subtitle={`آگهی‌های تازه‌ی ارائه‌دهنده‌ها در یک فهرست عمومی؛ همگام‌سازی هیچ صفحه‌ای قدیمی‌تر از ${toFaDigits(MAX_PROVIDER_SYNC_AGE_DAYS)} روز را واکشی نمی‌کند.`}
            actions={
              user ? (
          ...
          <Suspense key={JSON.stringify({ state, userId: user?.userId ?? null })} fallback={<JobsSkeleton />}>
            <JobsSection
...
async function JobsSection({
  ...
  if (page.filteredTotal === 0) {
    return (
      <EmptyState
        icon={<IconCompass />}
        title="با این فیلتر شغلی پیدا نشد"
        body="جست‌وجو را سبک‌تر کن یا بعد از همگام‌سازی بعدی دوباره نگاه کن."
        action={
...
function jobsHref(
  ...
  if (next.applied && next.applied !== "all") params.set("applied", String(next.applied));
