...
/**
 * سقفِ ماهانه‌ی هزینه‌ی AI — پیش‌فرض **خاموش** (۰ یعنی بی‌سقف).
 *
 * چرا خاموش: این سقف با *تخمینِ* داخلیِ قیمت سنجیده می‌شد، نه هزینه‌ی واقعی. جدولِ
 * قیمتِ داخلی صراحتاً «تخمینی» است و همگام‌سازیِ زنده‌اش کار نمی‌کرد، پس عددی که با
 * سقف مقایسه می‌شد ساختگی بود: در یک روز، متر داخلی ۲٬۱۰۵٬۲۷۹ تومان نشان داد در حالی
 * که برداشتِ واقعی از کیف‌پولِ 1xai ۷۸۶٬۷۹۲ تومان بود — و محصول خودش را با آن عددِ
 * نادرست متوقف کرد.
 *
 * حالا مرجع، کیف‌پولِ خودِ 1xai است: هزینه را همان‌جا حساب و برداشت می‌کند، پس نبودِ
 * موجودی گاردِ واقعی و دقیق است. اگر کسی عمداً سقفِ نرم بخواهد، این را در env ست کند.
 */
export const DEFAULT_AI_MONTHLY_BUDGET_USD = 0;
