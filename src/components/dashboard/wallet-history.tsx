  ...
  if (entries.length === 0) {
    return (
      <Card className="p-6">
        ...
        <p className="mt-3 rounded-xl border border-dashed border-border bg-card/50 px-4 py-5 text-center text-sm text-muted">
          ...
          return (
            <li key={e.id} className="flex items-center justify-between gap-3 py-3">
              <div className="min-w-0">
                ...
                <div className="ltr-nums mt-1 text-xs text-muted">{toFaDigits(faDate(e.createdAt))}</div>
              </div>
              <div className="text-left">
                ...
                <div className="ltr-nums text-xs text-muted">
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        ...
        body="به‌محض اولین استفاده از سرویس‌های هوش مصنوعی (تطبیق، انگیزه‌نامه، پردازشِ رزومه)، هزینه‌ی هر فراخوانی این‌جا فهرست می‌شود."
      />
    );
  }
        ...
        <table className="w-full min-w-[40rem] text-right text-sm">
          <thead>
            <tr className="border-b border-border text-xs text-muted">
              ...
              <th className="px-4 py-3 font-medium">توکن (ورودی/خروجی)</th>
              <th className="px-4 py-3 font-medium">هزینه (تومان)</th>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="transition-colors hover:bg-foreground/[0.02]">
                ...
                <td className="px-4 py-3">
                  <div className="ltr-nums">{r.modelId}</div>
                  <div className="text-xs text-muted">{providerLabel(r.provider)}</div>
                </td>
                <td className="ltr-nums px-4 py-3 text-muted">
                  {toFaDigits(r.promptTokens)} / {toFaDigits(r.completionTokens)}
                </td>
                <td className="ltr-nums px-4 py-3 font-bold">
