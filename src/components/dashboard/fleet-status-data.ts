...
/**
 * خواندنِ «وضعیتِ اپلای خودکارِ سرورِ اپلای» برای پنلِ premium کاربر (Track C).
 *
 * همه فقط-خواندنی و مقید به userId (قاعده‌ی ۴: داده‌ی هر کاربر فقط برای همان کاربر).
 * این لایه «مصرف‌کننده» است و چیزی نمی‌نویسد:
 *   • پلنِ کاربر را از plan-data می‌گیرد و قابلیتِ سرورِ اپلای (سقفِ IP) را تعیین می‌کند،
 *   • تعدادِ نودهای *تخصیص‌یافته به همین کاربر* را از هسته‌ی Foundation (listAssignments)
 *     می‌شمارد (کدام ورکرها برای او اپلای می‌کنند — هرگز نودِ کاربرِ دیگر)،
...
/** بسته‌ی داده‌ی پنلِ وضعیتِ اپلای خودکارِ سرورِ اپلای. */
export interface FleetStatusData {
  /** قابلیتِ سرورِ اپلایِ پلنِ کاربر (آیا ورکر دارد + سقفِ IP). */
  capability: PlanFleetCapability;
  /** تعدادِ نودهای ورکرِ هم‌اکنون‌تخصیص‌یافته به این کاربر. */
...
/**
 * همه‌ی داده‌ی پنلِ وضعیتِ سرورِ اپلای را می‌سازد: قابلیتِ پلن، تعدادِ نودهای تخصیص‌یافته،
