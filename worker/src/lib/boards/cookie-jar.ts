/**
 * یک cookie jarِ کوچکِ درون‌حافظه‌ای برای اپلایِ HTTPِ نود.
 *
 * نسخه‌ی دومی از همین فایل در کنترل‌پلین هست (`src/lib/apply/login/cookie-jar.ts`)
 * که درایورهای ورود از آن استفاده می‌کنند. دو پکیجِ جدا هستند و ماژول مشترک ندارند،
 * پس عمداً هرکدام نسخه‌ی خودش را دارد. این فایل کوچک و بی‌وابستگی است و رفتارش را
 * استانداردِ Set-Cookie تعیین می‌کند نه منطقِ ما، پس دور افتادنشان کم‌خطر است — برخلافِ
 * APPLY_SPEC که سلکتورهای متغیرِ هر سایت را نگه می‌داشت و واقعاً دور افتاد.
 *
