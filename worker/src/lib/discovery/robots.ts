/**
 * robots.txt — PORTED VERBATIM from the control plane (src/lib/apply/robots.ts),
 * which carries the full RFC 9309 test suite. The node now does the anonymous
 * public crawling, so it has to apply exactly the same rules; a second, looser
 * interpretation here would quietly turn "we honour robots.txt" into a claim that
 * only one of our two crawlers lives up to.
 *
 * Key policy (unchanged): 429/5xx on robots.txt = disallow everything and retry
 * later; 404/403/410 or a network error = no rules; the most specific rule wins.
 */
